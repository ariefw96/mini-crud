...
public interface CartRepository extends JpaRepository<Cart, Long> {
    ...
    @Query(value = "SELECT * FROM cart c WHERE lower(c.trx_id) = :trxId", nativeQuery = true)
    public List<Cart> getCartByTrxId(@Param("trxId") String trxId);

    @Query(value = "SELECT * FROM cart c WHERE lower(c.trx_id) = :trxId and c.item_id = :itemId", nativeQuery = true)
    public Optional<Cart> getItemOnCart(@Param("trxId") String trxId, @Param("itemId") Long itemId);

    @Modifying(clearAutomatically = true)
    @Query(value = "DELETE FROM cart WHERE lower(trx_id) = :trxId and item_id = :itemId", nativeQuery = true)
